...
`;
  ...
  let somethingWentWrong = `<img id="wentWrongImg" src="../assets/gif/oops-something-went-wrong.gif">`;
  ...
  async function getGames(
    ...
    /*-------Check if searchinput contains something------- */
    ...
    /*------this is just to see wich link is being fetched---------- */
    console.log("ESTA ES LA URL QUE ENTRO", link);

    try {
      const response = await fetch(link);

      /*check if is the first time fetching or I'm going for next page */
      if (nextGamesPage.length === 0) {
    ...
    if (searchQuery.length === 0) {
      ...
      try {
        response = await fetch(
          `https://api.rawg.io/api/games?key=c7b6f273e3984c5e83fb3f76702937f2&search=${searchQuery}&page_size=15`
        );
